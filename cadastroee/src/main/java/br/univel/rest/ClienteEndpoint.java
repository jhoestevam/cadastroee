...
import br.univel.rest.dto.ClienteDTO;
...
public class ClienteEndpoint {
	@PersistenceContext(unitName = "cadastroee-persistence-unit")
	private EntityManager em;

	@POST
	@Consumes("application/json")
	public Response create(ClienteDTO dto) {
		Cliente entity = dto.fromDTO(null, em);
		em.persist(entity);
	...
	@GET
	@Produces("application/json")
	public List<ClienteDTO> listAll(@QueryParam("start") Integer startPosition,
		...
		final List<Cliente> searchResults = findAllQuery.getResultList();
		final List<ClienteDTO> results = new ArrayList<ClienteDTO>();
		for (Cliente searchResult : searchResults) {
			ClienteDTO dto = new ClienteDTO(searchResult);
			results.add(dto);
		}
		return results;
	}
